...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json(
        successResponse(res, { mcqId: mcq.id, contestId: mcq.contest_id }, 201),
      );
    } catch (error: any) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const isCorrect =
        parsed.data.selectedOptionIndex === mcq.correct_option_index;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      //helper
      const addScore = (userId: number, points: number) => {
        const current = userScores.get(userId) || { name: "", totalPoints: 0 };
      ...
      //mcq scores
      contest.mcqs.forEach((mcq) => {
        mcq.submissions.forEach((submission) => {
          if (submission.is_correct) {
            addScore(submission.user_id, submission.points_earned);
          }
        });
      });

      //dsa scores
      contest.dsaProblems.forEach((problem) => {
        const bestSubmissions = new Map<number, number>();
        problem.submissions.forEach((submission) => {
          const currentBest = bestSubmissions.get(submission.user_id) || 0;
          if (submission.points_earned > currentBest) {
            bestSubmissions.set(submission.user_id, submission.points_earned);
          }
        });
        bestSubmissions.forEach((userId, points) => {
      ...
      for (const user of users) {
        const data = userScores.get(user.id);
        if (data) {
          data.name = user.name;
        }
      }
      ...
      const leaderboard = Array.from(userScores.entries())
        .map(([userId, data]) => {
          return { userId, name: data.name, totalPoints: data.totalPoints };
        })
      ...
      const rankedLeaderboard = leaderboard.map((entry, index) => {
        ...
        if (
          index > 0 &&
          entry.totalPoints === (leaderboard[index - 1] as any).totalPoints
        ) {
